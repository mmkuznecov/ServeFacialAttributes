...
def test_yolov8_face_detection_handler(mock_context):
    ...
    assert isinstance(postprocessed_output[0]["boxes"], list), "'boxes' should be a list"
    ...
    for box in postprocessed_output[0]["boxes"]:
        ...
        assert all(isinstance(value, float) for value in box), "Box values should be floats"
