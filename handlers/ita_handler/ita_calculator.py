...
class ITACalculator:
    ...
    def calculate_ita(self, img):
        ...
        for i, d in enumerate(rects):
            faces[i, :] = (d.left(), d.top(), d.right() - d.left(), d.bottom() - d.top())

        ITAs = []
        for i, rect in enumerate(rects):
            img_r, img_c, img_l = self.make_masked_face_image(faces, i, img, img.copy(), gray_img)
    ...
    def make_masked_face_image(self, faces, index, img, img_copy, gray_img):
        ...
        return np.where(mask_r == 255, img_filtered, mask_r), \
