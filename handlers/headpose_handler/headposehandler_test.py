...
from headposehandler import SixDRepNet360Handler
import os
...
def test_headpose_handler_with_real_weights(mock_context):
    ...
    for output in final_result:
        assert all(key in output for key in ['yaw', 'pitch', 'roll']), "Output missing expected keys"
        assert all(isinstance(output[key], float) for key in ['yaw', 'pitch', 'roll']), "Values should be float"
