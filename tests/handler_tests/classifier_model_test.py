...
import os
from src.handlers.classifiers_handler.customresnetclassifier import (
...
def test_process_image(sample_image, device):
    ...
    ), "Transformed image should be a tensor"
    assert transformed_image.shape == (
        3,
        224,
        224,
    ), "Transformed image shape should be (3, 224, 224)"
...
def test_predict_single_image_bald(bald_classifier, sample_image):
    ...
    assert prediction.shape == (
        1,
        bald_classifier.num_classes,
    ), f"Prediction shape should be (1, {bald_classifier.num_classes})"
...
def test_predict_batch_images_bald(bald_classifier, sample_image):
    ...
    assert predictions.shape == (
        4,
        bald_classifier.num_classes,
    ), f"Predictions shape should be (4, {bald_classifier.num_classes})"
...
def test_predict_label_bald(bald_classifier, sample_image, bald_mapping):
    ...
    assert (
        "not_bald" in predictions[0] and "bald" in predictions[0]
    ), "Predictions should contain 'not_bald' and 'bald' labels"
...
def test_predict_label_batch_bald(bald_classifier, sample_image, bald_mapping):
    ...
    assert (
        len(predictions) == 4
    ), "There should be one prediction for each image in the batch"
    for prediction in predictions:
        assert isinstance(prediction, dict), "Each prediction should be a dictionary"
        assert (
            "not_bald" in prediction and "bald" in prediction
        ), "Predictions should contain 'not_bald' and 'bald' labels"
...
def test_predict_single_image_race(race_classifier, sample_image):
    ...
    assert prediction.shape == (
        1,
        race_classifier.num_classes,
    ), f"Prediction shape should be (1, {race_classifier.num_classes})"
...
def test_predict_batch_images_race(race_classifier, sample_image):
    ...
    assert predictions.shape == (
        4,
        race_classifier.num_classes,
    ), f"Predictions shape should be (4, {race_classifier.num_classes})"
