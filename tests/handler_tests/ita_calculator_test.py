...
import numpy as np
import cv2
from src.handlers.ita_handler.ita_calculator import ITACalculator, extract_non_black, calc_ita
import os
...
def test_extract_non_black(sample_image):
    ...
    assert isinstance(non_black_pixels, np.ndarray), "Extracted non-black pixels should be a numpy array"
    assert non_black_pixels.shape[2] == 3, "Extracted pixels should have 3 color channels"
