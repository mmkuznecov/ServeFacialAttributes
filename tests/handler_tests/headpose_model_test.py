...
def test_sixdrepnet360_initialization():
    ...
    assert isinstance(
        model, SixDRepNet360
    ), "Model should be an instance of SixDRepNet360"
    assert isinstance(
        model.conv1, torch.nn.Conv2d
    ), "conv1 should be an instance of Conv2d"
    assert isinstance(
        model.linear_reg, torch.nn.Linear
    ), "linear_reg should be an instance of Linear"


def test_headpose_estimator_initialization(estimator):
    assert isinstance(
        estimator.model, SixDRepNet360
    ), "Model should be an instance of SixDRepNet360"
    assert estimator.device in [
        "cuda",
        "cpu",
...
def test_load_weights(estimator, weights_path):
    ...
    assert (
        estimator.model is not None
    ), "Model should be initialized after loading weights"
