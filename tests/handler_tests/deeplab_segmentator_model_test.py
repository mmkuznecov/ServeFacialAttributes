...
from src.handlers.deeplab_segmentator_handler.deeplab_segmentator import FaceSegmentationPredictor
...
def test_predictor_initialization(predictor):
    ...
    assert predictor.device in ["cpu", "cuda"], "Device should be either 'cpu' or 'cuda'"
    assert predictor.transform is not None, "Transform should be initialized"
    assert predictor.CLASS_COLOR_MAP is not None, "CLASS_COLOR_MAP should be initialized"
...
def test_predict_image_content(predictor, sample_image):
    ...
    assert result_array.shape == (sample_image.size[1], sample_image.size[0], 3), "Result shape should match input image shape"
    ...
    assert unique_colors.issubset(expected_colors), "Result should only contain colors from CLASS_COLOR_MAP"
