...
import numpy as np
import cv2
from src.handlers.dlib_segmentator_handler.dlib_segmentator import (
    DlibSegmentator,
...
def sample_image(sample_image_path):
    ...
    )  # Convert to RGB for consistency with dlib
...
def test_segment_face(segmentator, sample_image):
    ...
    assert (
        mask.shape == sample_image.shape[:2]
    ), "Mask shape should match input image shape"
...
def test_segment_face_content(segmentator, sample_image):
    ...
    for color in unique_colors:
        assert (
            color in expected_colors or color == 0
        ), f"Unexpected color value {color} in mask"
...
def test_no_faces(segmentator):
    ...
    assert np.all(
        mask == 0
    ), "Mask should contain only zero values as there are no faces"
